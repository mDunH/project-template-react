...
export function isOnlyMobile() {
    return /Android|webOS|iPhone|iPod|BlackBerry|IEMobile|Opera Mini|Nokia/i.test(
        navigator.userAgent
    );
}
...
export function getPlatform() {
    ...
    const platform = Object.keys(platformSize)
        .filter(key => platformSize[key] < size)
        .map(key => ({ key, value: platformSize[key] }))
